...
def create_view(include_user: bool) -> None:
    user_columns = """
            MAX(u.full_name) AS user_full_name,
            MAX(u.phone) AS user_phone,
""" if include_user else ""
    user_join = "LEFT JOIN users u ON b.user_id = u.id" if include_user else ""
...
def upgrade() -> None:
    create_view(include_user=True)


def downgrade() -> None:
    create_view(include_user=False)
